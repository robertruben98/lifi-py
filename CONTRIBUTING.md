# Contributing to lifi-py

Thanks for your interest in improving `lifi-py`. This guide covers the local
setup and the quality bar every change must clear.

## Development setup

```bash
git clone https://github.com/robertruben98/lifi-py
cd lifi-py
...
Run the live smoke tests against the real keyless API only when needed:
...
Maintainers: bump the version in `pyproject.toml` and `src/lifi_py/__init__.py`,
