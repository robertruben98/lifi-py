...
class LifiError(Exception):
    ...
    Catch this to handle any lifi-py-originated failure regardless of kind. Note
    that low-level transport failures (timeouts, connection errors) propagate as
    the underlying :class:`httpx.HTTPError` subclasses, not as ``LifiError``.
    """
...
class LifiAPIError(LifiError):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            message: Human-readable error message (the API's ``message`` field
                when available, otherwise a generated description).
            status_code: The HTTP status code returned by the API.
